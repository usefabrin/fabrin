...
func (a *App) runMakemigrations(ctx context.Context, out io.Writer) error {
	...
	for _, module := range order {
		...
		goSrc := renderMigrationGo(varName, version, strings.Join(describe, "; "), upStmts, downStmts)

		written := []string{}
		for path, data := range map[string][]byte{
			filepath.Join(dir, base+".go"):         []byte(goSrc),
		...
		if err := os.WriteFile(migPath, []byte(goSrc), 0o644); err != nil {
...
func renderMigrationGo(varName, version, name string, up, down []string) string {
	...
	return fmt.Sprintf(`// Code generated by fabrin makemigrations. Editing Up or Down by hand is
