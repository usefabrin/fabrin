...
| ID | Requirement | Status |
|---|---|---|
| FR-AUTH-1 | Framework-owned identity is created only after atomic email-OTP verification. Challenges use cryptographic secrets, protected verifiers, bounded expiry/attempt/send budgets, replay resistance, and enumeration-safe responses. Passwords, social login, and passkeys are deferred from v1. | in progress |
...
| FR-AUTH-5 | Auth defaults and ports satisfy the reviewed threat model, including enumeration resistance, distributed abuse budgets, recovery, invitation-only mode, secret-free durable audit, key rotation, cancellation, and fail-closed dependency behavior. | in progress |
