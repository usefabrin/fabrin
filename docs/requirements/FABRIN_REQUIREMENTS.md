...
| FR-AUTH-7 | Auth transport and sessions enforce cookie/native separation, CSRF, rotation/revocation, bounded input and secret-free audit behavior. Native bearer and secure browser-cookie handlers implement bounded strict input, purpose separation, no-store responses and transport-specific protected-route middleware. Browser flows add exact-origin credentialed CORS, reusable unsafe-route CSRF middleware, challenge binding and credential-free verification JSON; broader policy and audit remain. | in progress |
