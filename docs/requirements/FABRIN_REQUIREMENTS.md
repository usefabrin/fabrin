...
| FR-ORM-2 | GORM is the shipped default adapter — a documented adapter pattern and a worked example, not an exported Fabrin type returning `*gorm.DB`. See [ADR 0002](../adr/0002-database-sql-is-the-orm-seam.md). The pattern and the worked example are `examples/hello/orders` (ORM-010/011), over `database/sql` and SQLite; the GORM adapter itself is unwritten. | in progress |
