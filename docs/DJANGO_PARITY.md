...
| Each migration serializes the model state it produces; `makemigrations` reconstructs "before" by replaying them | `orm.Snapshot` + `ReplayState` — deterministic versioned JSON, legacy constraint semantics decoded explicitly, hand-written steps carry the last known state forward | ✅ F2 (codec) / 📋 F2 (per-version generator state) |
| The migration autodetector (`django.db.migrations.autodetector`) | Public `migratediff` operations — tables, fields, type/length, nullability, primary keys, UNIQUE, and indexes are independently diffed in dependency order | ✅ F2 |
