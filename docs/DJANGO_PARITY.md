...
| Each migration serializes the model state it produces; `makemigrations` reconstructs "before" by replaying them | `orm.Snapshot` + `ReplayState` — deterministic versioned JSON, cumulative state per generated version, explicit legacy semantics, and hand-written carry-forward | ✅ F2 |
