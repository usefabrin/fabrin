...
| Each migration serializes the model state it produces; `makemigrations` reconstructs "before" by replaying them | `orm.Snapshot` + `ReplayState` — deterministic JSON codec, ADR 0006 flags withheld until the versioned codec slice, hand-written steps carry the last known state forward | ✅ F2 (mechanism) / 📋 F2 (constraint codec) |
