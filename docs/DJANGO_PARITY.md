...
| `makemigrations` / `migrate` | `./myapp makemigrations` writes per-module Go + state files; `./myapp migrate [-to]` runs them — commands, format, engine, and recorded state all exist | ✅ F2 |
| Per-app `migrations/` directories | `<module>/migrations/` — one migration per owning module, versions from a shared fixed-width clock, hand-written steps carried forward | ✅ F2 |
| Each migration serializes the model state it produces; `makemigrations` reconstructs "before" by replaying them | `orm.Snapshot` + `ReplayState` — deterministic versioned JSON, explicit legacy decoding, hand-written carry-forward, and cumulative sidecars at every generated version | ✅ F2 |
| The migration autodetector (`django.db.migrations.autodetector`) | Public `migratediff` operations — tables, fields, type/length, nullability, primary keys, UNIQUE, and indexes are independently diffed in dependency order; compatible drop/add pairs are confirmed interactively as renames, while noninteractive runs refuse rather than guess | ✅ F2 |
...
| Sessions | Server-side memory/Redis sessions, protected-route middleware, bounded pre-auth/session CSRF, secure cookies, and user/admin identity-wide revocation | 🚧 v1 |
