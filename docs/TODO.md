...
- [x] Recorded model state per migration — the "before" `makemigrations` diffs
      ...
      all-nullable semantics; unknown marked versions fail closed. When one run
      changes several modules, each sidecar records the cumulative schema its
      own ordered version produces, and replay reaches the declared final state.
...
- [ ] Permissions, groups, and ownership callbacks shared by REST and admin.
      (FR-AUTH-3, #113)
- [ ] Framework-owned identity with application-owned related profile data.
      (FR-AUTH-4)
