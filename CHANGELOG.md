...
- **Breaking:** `orm` column-type constants now carry the `Type` prefix:
  ...
  as aliases, and `schema.Generate` now emits the prefixed names; source it
  generated before this change references the removed constants and must be
  regenerated. ([#122](https://github.com/usefabrin/fabrin/issues/122))
