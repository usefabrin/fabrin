...
| OTP-004 | Public local OTP orchestration reuses the protected challenge proof and mail capture, then resolves a stable identity only after atomic verification. | `auth/auth_test.go::TestService_ConcurrentConsumeAndIdentityResolution` |
| OTP-005 | Local address and source send budgets survive challenge replacement. | `auth/auth_test.go::TestService_ResendReplacesChallengeWithoutResettingBudgets` |
...
| SES-005 | Trusted privilege writers revoke every session by identity ID; Redis deletes the shared identity index atomically | `auth/session_test.go::TestSessionManager_RevokeIdentityRejectsEverySession`, `authredis/authredis_test.go::TestStore_RedisRevokesSessionsByIdentity` |
